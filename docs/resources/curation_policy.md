...
`share_with_federation` can be enabled only when `scope` is `all_repos` or `pkg_types` and requires Curation Federation (Xray 3.143.x and Artifactory 7.146.x).
...
- `share_with_federation` (Boolean) When true, shares the policy with federated Xray controllers. Supported only for all_repos and pkg_types scopes. Requires Curation Federation (Xray 3.143.x and Artifactory 7.146.x). Defaults to false.
