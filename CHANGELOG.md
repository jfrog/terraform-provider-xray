...
* resource/xray_curation_policy: Fix `Error: Duplicate Set Element` on import/read when a policy holds content-identical `waivers` (or `label_waivers`) that differ only by the server-assigned `id`/`created_at` (not modeled by the provider). Such duplicates are common when waivers are added out-of-band via the UI/API, and previously made the policy unmanageable because the crash occurred during `Read`/refresh/import before any `apply` could rewrite it. `fromAPIModel` now deduplicates waivers on their full provider-visible content before building the set. Issue: [#457](https://github.com/jfrog/terraform-provider-xray/issues/457)
