...
* resource/xray_security_policy, resource/xray_license_policy, resource/xray_operational_risk_policy: Switch `rule` and `criteria` attribute types from `Set` to `List` to maintain rule ordering and improve performance with large number of rules. PR: [#372](https://github.com/jfrog/terraform-provider-xray/pull/372)
...
* resource/xray_vulnerabilities_report, resource/xray_violations_report, resource/xray_licenses_report, resource/xray_operational_risks_report, resource/xray_exposures_report: Switch pattern attributes (`include_path_patterns`, `exclude_path_patterns`, `include_patterns`, `exclude_patterns`, `names`, `keys`, `include_key_patterns`, `exclude_key_patterns`) from `Set` to `List` to improve performance with large number of patterns. PR: [#372](https://github.com/jfrog/terraform-provider-xray/pull/372)

* resource/xray_vulnerabilities_report, resource/xray_violations_report, resource/xray_licenses_report, resource/xray_operational_risks_report, resource/xray_exposures_report: Add validation for `include_path_patterns` and `exclude_path_patterns` to reject patterns starting with `/` or containing `@` symbol. PR: [#372](https://github.com/jfrog/terraform-provider-xray/pull/372)
...
## 3.1.2 (Oct 16, 2025). Tested on JFrog Platform 11.2.4 (Artifactory 7.117.18, Xray 3.124.28, Catalog 1.26.0) with Terraform 1.13.4 and OpenTofu 1.10.6
...
* resource/xray_security_policy: Switch `vulnerability_ids` attribute type from `TypeSet` to `TypeList` to resolve performance issue with large number of CVE's. PR: [#334](https://github.com/jfrog/terraform-provider-xray/pull/334)
...
* resource/xray_ignore_rule: Added `release_bundles_v2` attribute to support creating ignore rule for Release Bundles V2. PR: [#319] (https://github.com/jfrog/terraform-provider-xray/pull/319)
