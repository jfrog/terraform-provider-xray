...
const minVersionForNewFeatures = "3.130.0"
const runtimeFilterEnvVar = "XRAY_RUNTIME_FILTER_ENABLED"
...
func TestAccViolationsReport_Repository(t *testing.T) {
	...
	t.Run("violations_repository_by_name", func(t *testing.T) {
		...
		version, err := util.CheckXrayVersion(acctest.GetTestResty(t), minVersionForNewFeatures, "")

		var extendedFilters string
		if err == nil && version >= minVersionForNewFeatures {
			extendedFilters = `
						ca_filter {
...
func TestAccViolationsReport_ReleaseBundleV2(t *testing.T) {
	...
	t.Run("violations_release_bundles_v2_by_pattern", func(t *testing.T) {
		...
		version, err := util.CheckXrayVersion(acctest.GetTestResty(t), minVersionForNewFeatures, "")
		if err != nil || version < minVersionForNewFeatures {
			t.Skipf("Skipping test: requires Xray version %s or higher", minVersionForNewFeatures)
			return
		}
...
func TestAccViolationsReport_Invalid(t *testing.T) {
	...
	t.Run("conflicting_severities_and_cvss", func(t *testing.T) {
		// Skip test if Xray version is lower than 3.130.0
		version, err := util.CheckXrayVersion(acctest.GetTestResty(t), minVersionForNewFeatures, "")
		if err != nil || version < minVersionForNewFeatures {
			t.Skipf("Skipping test: requires Xray version %s or higher", minVersionForNewFeatures)
			...
			Steps: []resource.TestStep{
				{
					...
					`, name, reportName),
					PlanOnly:    true,
					ExpectError: regexp.MustCompile(`Error running non-refresh plan: exit status 1\n\nError: Invalid Configuration\n\n\s+with xray_violations_report\..*,\n\s+on terraform_plugin_test\.tf line \d+, in resource "xray_violations_report".*\n.*\n\nCannot use both severities \(under filters\) and cvss_score \(under\nsecurity_filters\) together from Xray version`),
				},
			},
		})
	})
