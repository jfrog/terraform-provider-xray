...
// Don't kill yet, use it as HCL examples later on
func TestAccVulnerabilitiesReport1(t *testing.T) {
		...
		resource "xray_vulnerabilities_report" "report" {
			...
			resources {
				repository {
					...
					exclude_path_patterns 	= ["pattern2"]
				}
				repository {
					name 					= "reponame1"
					include_path_patterns 	= ["pattern1"]
		...
		resource "xray_vulnerabilities_report" "report" {
			...
			resources {
				projects {
					...
					//include_key_patterns			= ["include1", "include2"]
					number_of_latest_versions	= 2
				}
			}
			filters {
	...
	const reposLicenses = `
		resource "xray_licenses_report" "report" {
			...
			filters {
				...
				//license_patterns 	= ["*Apache*","The Apache*"]
				scan_date {
					start 					= "2020-06-29T12:22:16Z"
					end						= "2020-07-29T12:22:16Z"
				}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: reposLicenses,
				//Check:  resource.TestCheckResourceAttr("xray_vulnerabilities_report.report", "name", "test-report"),
				Check: resource.TestCheckResourceAttr("xray_licenses_report.report", "name", "test-license-report"),
				//Check: resource.TestCheckResourceAttr("xray_violations_report.report", "name", "test-violations-report"),
				//Check: resource.TestCheckResourceAttr("xray_operational_risks_report.report", "name", "test-report"),
			},
		},
	})
}
